...
// ── Inline MCQ helpers (mirrored from create-form) ────────────────────────────
...
const imageValueForInlineMcq = (image) => {
  if (isImageFile(image)) return image;
  return typeof image === "string" && image.trim() ? image : "null_url";
};

const getGroupInlineMcqImageFieldName = (
  assessmentIndex,
  sectionIndex,
  mcqIndex,
  imageField,
) =>
  `newAssessments_${assessmentIndex}_sections_${sectionIndex}_newMcqProblems_${mcqIndex}_${imageField}`;

const buildGroupAssessmentFormData = (payload) => {
  const formData = new FormData();

  const normalizedNewAssessments = (payload.newAssessments || []).map(
    (assessment, assessmentIndex) => ({
      ...assessment,
      sections: (assessment.sections || []).map((section, sectionIndex) => ({
        ...section,
        newMcqProblems: (section.newMcqProblems || []).map((mcq, mcqIndex) => {
          ...
          MCQ_IMAGE_FIELDS.forEach((imageField) => {
            ...
            if (isImageFile(imageValue)) {
              formData.append(
                getGroupInlineMcqImageFieldName(
                  assessmentIndex,
                  sectionIndex,
                  mcqIndex,
                  imageField,
  ...
  formData.append(
    "assessmentData",
    JSON.stringify({ ...payload, newAssessments: normalizedNewAssessments }),
  );
...
const mapFormQuestionToInlineMcq = (question) => ({
  ...
  correctAnswer: String(question?.answerKey || "").trim().toLowerCase(),
...
const hasManualMcqValue = (value) => {
  ...
  return normalized !== "" && normalized !== "null_url";
};
...
const isCompleteManualMcq = (mcq) => {
  const correctAnswer = String(mcq?.correctAnswer || "").trim().toLowerCase();
  return (
...
const getFilledManualMcqs = (mcqs) =>
  (mcqs || []).filter(hasManualMcqContent).map((mcq) => ({
    ...mcq,
    correctAnswer: String(mcq.correctAnswer || "").trim().toLowerCase(),
...
const mapInlineMcqToFormQuestion = (mcq) => ({
  question: { statement: mcq?.question || "", image: null },
  ...
  answerKey: String(mcq?.correctAnswer || "").trim().toLowerCase(),
  explanation: mcq?.explanation || "",
});

const getManualMcqDefaultValues = (mcqs) => {
  if (Array.isArray(mcqs) && mcqs.length > 0) {
    return { questions: mcqs.map(mapInlineMcqToFormQuestion) };
  }
  return { questions: [addQuestionFormDefaultObject] };
};
...
const ManualMcqProblemsForm = ({ mcqs, setMcqs, type, subType }) => {
  const methods = useForm({ defaultValues: getManualMcqDefaultValues(mcqs) });
...
const CreateGroupAGHAssessment = ({ data, type, subType }) => {
  const queryClient = useQueryClient();
  const navigate = useNavigate();
  const location = useLocation();
  const existingGroup = location.state;
  ...
  const fetchExistingGroupDetails = async () => {
    const res = await axiosInstance.get(
      `${import.meta.env.VITE_BASE_URL}/v1/agh-assessments/admin/assessments/group/${existingGroup.groupId}`,
      {
        headers: {
  ...
  const { data: existingGroupDetails } = useQuery({
    queryKey: ["agh-existing-group-details", existingGroup?.groupId],
    queryFn: fetchExistingGroupDetails,
    enabled: !!existingGroup?.groupId && !!token,
  });

  const excludedGroupStudentIds = useMemo(() => {
    if (!existingGroup?.groupId) return [];

    return (existingGroupDetails?.scheduledForUsers || [])
      .map((student) => student?._id || student)
      .filter(Boolean);
  }, [existingGroup?.groupId, existingGroupDetails]);
  ...
  const {
    ...
    defaultValues: data
      ? {
          groupName:
            existingGroup?.groupName || data?.groupName || data?.name || "",
          ...data,
        }
      : {
  ...
  const onValid = (formData) => {
    ...
    if (formData.isSectionWise === "yes") {
      const sumOfSections = formData.sections.reduce((sum, section) => {
        return sum + (Number(section.sectionDuration) || 0);
      }, 0);
      ...
      if (Number(formData.duration) !== sumOfSections) {
        toast.error(
          `Total duration (${formData.duration} min) must equal sum of all section durations (${sumOfSections} min)`,
        );
        return;
      }
    }

    const sectionsWithFilledManualMcqs = formData.sections.map((section) => ({
      ...section,
    ...
    const assessmentPayload = {
      ...
      isSectionWise: formData.isSectionWise,
      startDateTime: formData.startDateTime,
      endDateTime: formData.endDateTime,
      scheduledForUsers: selectedStudents.map(getId),
      sections: sectionsWithFilledManualMcqs.map((section) => ({
        ...section,
        codingProblems: (section.codingProblems || []).map(getId),
        mcqProblems: (section.mcqProblems || []).map(getId),
        sqlProblems: (section.sqlProblems || []).map(getId),
        newMcqProblems: section.newMcqProblems,
      })),
    };

    mutate(
      buildGroupAssessmentFormData({
        groupName: formData.groupName,
        newAssessments: [assessmentPayload],
        existingAssessmentIds: [],
      }),
    );
  };

  const createGroupAssessment = async (formData) => {
    if (existingGroup?.groupId) {
      const res = await axiosInstance.patch(
        `${import.meta.env.VITE_BASE_URL}/v1/agh-assessments/admin/assessments/group/${existingGroup.groupId}/add`,
        formData,
    ...
    const res = await axiosInstance.post(
      `${import.meta.env.VITE_BASE_URL}/v1/agh-assessments/admin/assessments/group`,
      formData,
      { headers: { Authorization: `Bearer ${token}` } },
    );
    return res.data;
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: createGroupAssessment,
    onSuccess: () => {
        ...
        <InputWrapper>
          ...
          <InputContainerWithOutIcon>
            <input
              ...
              type="text"
              readOnly={!!existingGroup?.groupId}
              style={{
                backgroundColor: existingGroup?.groupId ? "#e8f0fe" : undefined,
                cursor: existingGroup?.groupId ? "not-allowed" : "text",
              }}
            />
          </InputContainerWithOutIcon>
          {errors?.groupName && (
        ...
        <div>
          ...
          <div style={{ border: "1px solid" }}>
            <UsersSelection
              ...
              excludedUserIds={excludedGroupStudentIds}
            />
          </div>
        </div>
...
export default CreateGroupAGHAssessment;
