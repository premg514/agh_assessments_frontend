...
const TechnicalMCQPendingQuestionsPage = () => {
  ...
  useEffect(() => {
    const fetchTechnicalMCQPendingQuestionsSuperAdminJunior = async (
      testId
    ) => {
      try {
        const getData = await axiosInstance.get(
          import.meta.env.VITE_BASE_URL +
            `/v1/super-admin-junior/getTechnicalPendingMCQQuestions`,
          {
            params: {
              testId,
              page: currentPage,
    ...
    if (testId) {
      fetchTechnicalMCQPendingQuestionsSuperAdminJunior(testId);
    } else {
      setError("Test ID is required");
    }
