...
import { convertDataIntoQuestionPreviewFormat } from "../../../../test-list-page/utils/functions";
...
const ViewPendingQuestionsPage = () => {
  ...
  useEffect(() => {
    const fetchAptitudePendingQuestionsSuperAdminJunior = async (testId) => {
      try {
        const getData = await axiosInstance.get(
          import.meta.env.VITE_BASE_URL +
            `/v1/super-admin-junior/getAllAptitudeELearningPendingQuestions`,
          {
            params: {
              testId,
              page: currentPage,
    ...
    if (testId) {
      fetchAptitudePendingQuestionsSuperAdminJunior(testId);
    } else {
      setError("Test ID is required");
    }
    ...
    try {
      const res = await axiosInstance.post(
        import.meta.env.VITE_BASE_URL +
          ...
          "/approveElearningTestAptitudeQuestions",
        {
          aptitudeELearningTestId: testId,
          superAdminJuniorId: superAdminJuniorId,
          otp: data.otp,
        },
        {
  ...
  const handleClickReject = async (data) => {
    ...
    try {
      const res = await axiosInstance.post(
        import.meta.env.VITE_BASE_URL +
          "/v1/super-admin-junior" +
          "/rejectElearningTestAptitudeQuestions",
        {
          aptitudeELearningTestId: testId,
          superAdminJuniorId: superAdminJuniorId,
          reason: data.reason,
          ...
          component={
            componentName === "ACCEPT" ? (
              <ConfirmationComponent
                detail={"Have you check every mcq question ?"}
...
export default ViewPendingQuestionsPage;
