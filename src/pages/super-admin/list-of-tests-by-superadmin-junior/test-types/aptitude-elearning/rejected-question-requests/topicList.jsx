...
import { convertFetchedDataToSidebarDesiredFormat } from "../../../../test-list-page/utils/functions";
...
const TopicList = ({ dataRoute }) => {
  ...
  useEffect(() => {
    const fetchSidebarData = async () => {
      ...
      try {
        const getData = await axiosInstance.get(
          import.meta.env.VITE_BASE_URL + `${dataRoute}`,
          {
            params: {
        ...
        // Convert the fetched data into the desired format for the sidebar
    ...
    fetchSidebarData();
  }, [dataRoute, token, superAdminJuniorId]);
                  ...
                  sidebarData[key].map((item1, index1) => {
                    return (
                      <SubTopic
                        item1={item1}
                        key={index1}
                        navigateRoute={`${location.pathname}/${item1.id}/mcq`}
                      />
                    );
                  })}
              </FlexColumnContainerSubTopic>
