    ...
    try {
      const getData = await axiosInstance.get(
        import.meta.env.VITE_BASE_URL +
          ...
          `/getListOfTechnicalELearningCodingPendingQuestionsRequestsTopics`,
        {
          params: {
      ...
      toast.error("Cannot get All Aptitude tests");
      console.log(error);
    } finally {
                  ...
                  sidebarData[key].map((item1, index1) => {
                    return (
                      <SubTopic
                        item1={item1}
                        key={index1}
                        navigateRoute={`/super-admin-junior/technical-elearning/questions/pending/${item1.id}/coding`}
                      />
                    );
                  })}
              </FlexColumnContainerSubTopic>
