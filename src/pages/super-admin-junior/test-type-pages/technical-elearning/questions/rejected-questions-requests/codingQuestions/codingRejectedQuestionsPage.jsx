...
import CodingTestEditComponent from "../../../../../../super-admin/coding-test-edit/coding-test-edit-component";
import FlashoutPageComponent from "../../../../../../../component/flash-out-page/flash-out-page-component";
import ConfirmationComponent from "../../../../../../../component/confirmation/confirmation-component";
import { getCorrectUrlForDeleteCodingQuestions } from "../../../../../../super-admin/test-list-page/utils/functions";
...
const TechnicalCodingRejectedQuestionsPage = () => {
  ...
  // below variable related to  TechnicalELearningQuestionsSuperadminjunior Schema
  const [
    technicalELearningCodingQuestionSchemaId,
    setTechnicalELearningCodingQuestionSchemaId,
  ] = useState(null);
  ...
  const handleDeleteQuestionSuperAdmin = async (
    testId,
    questionId,
    endPoint
  ) => {
  ...
  useEffect(() => {
    const fetchTechnicalELearningRejectedCodingQuestionsSuperAdminJunior =
      async (testId) => {
        try {
          const res = await axiosInstance.get(
            import.meta.env.VITE_BASE_URL +
              `/v1/super-admin-junior/getTechnicalELearningCodingRejectedQuestions`,
            {
              params: {
                testId,
                page: currentPage,
          ...
          setTechnicalELearningCodingQuestionSchemaId(
            res.data.technicalELearningCodingQuestionSchemaId
          );
    ...
    if (testId) {
      fetchTechnicalELearningRejectedCodingQuestionsSuperAdminJunior(testId);
    } else {
      setError("Test ID is required");
    }
  ...
  const handleClickYesOnReRequestModal = async () => {
    const taostId = toast.loading("Please wait...");
    try {
      const getQuestion = await axiosInstance.post(
        import.meta.env.VITE_BASE_URL +
          `/v1/super-admin-junior/reRaiseTechnicalELearningCodingQuestionsMergeRequest`,
        {
          technicalELearningCodingQuestionSchemaId:
            technicalELearningCodingQuestionSchemaId,
        },
  ...
  const fetchTechnicalELearningCodingRejectedQuestionsReasonSuperAdminJunior =
    async (testId) => {
      try {
        const res = await axiosInstance.get(
          import.meta.env.VITE_BASE_URL +
            `/v1/super-admin-junior/getTechnicalELearningCodingRejectedQuestionsReason`,
          {
            params: {
              testId,
            },
  ...
  useEffect(() => {
    if (technicalELearningCodingQuestionSchemaId) {
      fetchTechnicalELearningCodingRejectedQuestionsReasonSuperAdminJunior(
        technicalELearningCodingQuestionSchemaId
      );
    }
  }, [technicalELearningCodingQuestionSchemaId]);
          ...
          component={
            componentName === "Trash" ? (
              <ConfirmationComponent
                ...
                onClick={() => {
                  let endPoint = getCorrectUrlForDeleteCodingQuestions(
                    "Technical",
                    "eLearning",
                    user?.accountType
                  );
                  ...
                  handleDeleteQuestionSuperAdmin(
                    testId,
              ...
              />
            ) : componentName === "EditCodingQuestion" ? (
              <CodingTestEditComponent
                data={questions?.find(
              ...
              <BoxButtonsContainer className="width-fit">
                <BoxButton
                  handleClick={(e) => {
                    ...
                    setComponentName("EditCodingQuestion");
                    setPopupbox(true);
                  }}
                  color={"#09AFBB"}
                  bgColor={"#D1F1FE"}
